...
def prepare_model(data):
    ...
    CA1_cell.print_dend_params(dend_name='dend5_00')
    data.Elocs = lb.genDendLocs(data.locDend, 3, [0.9, 0.96])
    data.Ensyn = 3
    CA1_cell.add_synapses_on_spines(data=data, dend_name='dend5_00')
    ...
    CA1_cell.set_dendritic_voltage_vectors(sections=['dend5_00'])
...
def figures():
    ...
    fs = FiguresShower(setting=setting, save_figures=False,
                       path_saving='recordings/dend5_00/3_synapses_distal/',
                       path_recordings='recordings/dend5_00/3_synapses_distal/')
    fs.show_somatic_voltage()
    fs.show_dendritic_voltage(threshold=-37)
    fs.show_voltage_on_spines(threshold=-37)
    fs.show_average_weights_change(secs=['dend5_00'], keys=['dend5_00'], baseline=12000)
    fs.show_weights_distance(start=12000, stop=29000, synapse_type='perforated')


if __name__ == '__main__':
    ioh = IOHelper(path_saving='./recordings/dend5_00/3_synapses_distal/',
