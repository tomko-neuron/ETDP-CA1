...
def prepare_model(data):
    ...
    CA1_cell.print_dend_params(dend_name='dend2_010100')
    data.Elocs = lb.genDendLocs(data.locDend, 3, [0.88, 0.96])
    data.Ensyn = 3
    CA1_cell.add_synapses_on_spines(data=data, dend_name='dend2_010100')
    CA1_cell.connect_ns_terminals()
    CA1_cell.set_Makara_LTP(num_synapses=4, start=500, interval=2000, num=6, protocol=False)
    CA1_cell.set_Makara_LTP(num_synapses=8, start=12000, interval=333.33, num=50, protocol=True)
    CA1_cell.set_Makara_LTP(num_synapses=4, start=29000, interval=2000, num=6, protocol=False)
    CA1_cell.set_dendritic_voltage_vectors(sections=['dend2_010100'])
...
def figures():
    ...
    fs = FiguresShower(setting=setting, save_figures=False,
                       path_saving='./recordings/dend2_010100/3_synapses_distal/',
                       path_recordings='./recordings/dend2_010100/3_synapses_distal/')
    ...
    fs.show_average_weights_change(secs=['dend2_010100'], keys=['dend2_010100'], baseline=12000)
    fs.show_weights_distance(start=12000, stop=29000, synapse_type='perforated')


if __name__ == '__main__':
    ioh = IOHelper(path_saving='./recordings/dend2_010100/3_synapses_distal/',
